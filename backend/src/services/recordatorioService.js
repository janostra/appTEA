...
export class RecordatorioService {
  ...
  async editarRecordatorio(recordatorioID, datosActualizados) {
    try {
      const recordatorioExistente = await prisma.recordatorio.findUnique({
      ...
      const recordatorioEditado = await prisma.recordatorio.update({
        where: { ID: recordatorioID },
        data: {
          ...datosActualizados,
          hora: new Date(datosActualizados.hora),
        },
      })

      return recordatorioEditado
    } catch (error) {
